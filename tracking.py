...
cap= cv2.VideoCapture('vids/test2.mov')
HIST_SIZE=10
HIST_THRESH=100
...
def trackObject(location,previous,now,id):
    p=0
 ...
 #   center=start[0]+size[0]/2,start[1]+size[1]/2
    ...
    for angle in [0]:
        for step in [8,4,2,1]:
            start,stop,score=SS(step,template,now,start)
    return (start,stop)
...
def SS(step,template,image,location):
    ...
    for x in options:
        for y in options:
            if location[1]+x>0 and location[1]+s1+x<image.shape[1] and location[0]+y>0 and location[0]+s1+y<image.shape[1]:
...
while(True):
    ...
    for i in range(0,len(objects)):
        ...
        objects[i]=trackObject(loc,previous,now,i)
